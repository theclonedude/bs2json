...
class bs2json:
	def convert(self, element, json={}):
		if not type(json) == dict:
			raise TypeError(
				f"convertAll(x,y) `y` argument must be a {dict.__name__} not {type(json).__name__}")
		if type(element) == Element.Tag:
			json[element.name] = self.toJson(element)
			return json
		else:
			raise TypeError(
				f"convert(x,y) `x` argument must be a {Element.__name__} not {type(element).__name__}")

	def convertAll(self, elements, L=None):
		List = L if L is not None else []
		if not type(List) == list:
			raise TypeError(
				f"convertAll(x,y) `y` argument must be a {list.__name__} not {type(List).__name__}")
		if type(elements) == ResultSet:
			for element in elements:
				List.append(self.toJson(element))
			return List
		elif type(elements) == list:
			if len(elements) > 0 and type(elements[0]) == Element.Tag:
				for element in elements:
					List.append(self.toJson(element))
				return List
		else:
			raise TypeError(
				f"convertAll(x,y) `x` argument must be a {ResultSet.__name__} not {type(elements).__name__}")
